            ...
            if id == 0 or id == 1:
                ...
                for i, node in enumerate(nodes):
                    if node in temp:
                        id_list.append(i)
                concat_id_list.append(id_list)
    return concat_id_list

def concat(concat_id_list, label_embedding, span_embedding):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    matrix = torch.zeros(768, 402, dtype=torch.float32, device=device)
    span_emb = span_embedding(torch.tensor([0, 1], device=device))
    B_vector = span_emb[0]
    I_vector = span_emb[1]
    j = 0
    for i, id_list in enumerate(concat_id_list):
        label_emb = label_embedding(torch.tensor(id_list, device=device))
        if i == 0 or i == 1:
            label_vector = label_emb[0]
            zero_vector = torch.zeros(576, dtype=torch.float32, device=device)
            label_vector = torch.cat((label_vector, zero_vector))
            matrix[:, j] = label_vector
            j += 1
        elif i >= 42:
            if len(id_list) == 1:
                label_vector = label_emb[0]
                zero_vector = torch.zeros(384, dtype=torch.float32, device=device)
                label_vector = torch.cat((label_vector, zero_vector))
                matrix[:, j] = torch.cat((B_vector, label_vector))
                matrix[:, j + 1] = torch.cat((I_vector, label_vector))
                j += 2
            elif len(id_list) == 2:
                label_vector = torch.cat((label_emb[0], label_emb[1]))
                zero_vector = torch.zeros(192, dtype=torch.float32, device=device)
                label_vector = torch.cat((label_vector, zero_vector))
                matrix[:, j] = torch.cat((B_vector, label_vector))
                matrix[:, j + 1] = torch.cat((I_vector, label_vector))
                j += 2
            elif len(id_list) == 3:
                label_vector = torch.cat((label_emb[1], label_emb[1]))
                label_vector = torch.cat((label_vector, label_emb[2]))
                matrix[:, j] = torch.cat((B_vector, label_vector))
                matrix[:, j + 1] = torch.cat((I_vector, label_vector))
                j += 2
...
class Ner(New_BertForTokenClassification):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, valid_ids=None,
        ...
        concat_id_list = self.concat_id_list
        W = concat(concat_id_list, self.label_embedding, self.span_embedding)
        # print(W.size())
